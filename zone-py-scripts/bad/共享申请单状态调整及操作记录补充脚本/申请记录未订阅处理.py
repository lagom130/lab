...
if __name__ == '__main__':
    pipeline2 ={'status': {'$in': ['unSubscrib']}}
        ...
        for doc in result:
            if not doc["data_id"] in all_ids:
                ...
                if True:
                    count = count+1
                    print(count)
                    if apply['apply_type'] == 'API':
                        new_doc = doc
                        new_doc.pop('_id')
                        new_doc['data_status'] = doc['data_status'].split('|')[0]+"|finished"
                        if 'creater_dep_name' in apply:
                            new_doc['content'] = apply['creater_dep_name']+"已订阅"
                            new_doc['operator_name'] = apply['creater_dep_name']
                            new_doc['operator_id'] = apply['creater_dep_id']
                        else:
                            new_doc['content'] = "已订阅" 
                        ...
                        new_doc = doc
                        new_doc.pop('_id')
                        new_doc['data_status'] = doc['data_status'].split('|')[0]+"|unSend"
                        if 'creater_dep_name' in apply:
                            new_doc['content'] = apply['creater_dep_name']+"已订阅资源"
                            new_doc['operator_name'] = apply['creater_dep_name']
                            new_doc['operator_id'] = apply['creater_dep_id']
                        else:
                        ...
                        if 'data_creater_dep_name' in apply:
                            ...
                            new_doc['operator_id'] = apply['data_creater_dep_id']
                        else:
                            new_doc['content'] = "推送资源" 
                        if 'created_time' in doc:
                            if type(doc['created_time']) is str:
                                day_time= datetime.strptime(doc['created_time'], '%Y-%m-%d %H:%M:%S') + timedelta(days=2)
                                min_time = day_time + timedelta(minutes=20)
                                new_doc['created_time']=datetime.strftime(min_time, '%Y-%m-%d %H:%M:%S')
                            else:
                                created_time = datetime.strftime(doc['created_time'], '%Y-%m-%d %H:%M:%S')
                                day_time= datetime.strptime(created_time, '%Y-%m-%d %H:%M:%S') + timedelta(days=2)
                                min_time = day_time + timedelta(minutes=20)
                                new_doc['created_time']=datetime.strftime(min_time, '%Y-%m-%d %H:%M:%S')
                        else:
                            new_doc['created_time']=datetime.strftime(datetime.now, '%Y-%m-%d %H:%M:%S')

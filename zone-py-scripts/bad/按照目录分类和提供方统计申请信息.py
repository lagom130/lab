...
###
code_field_name ='base_info_resource_code'
codes = ['3070150500101', '3070150500105']
info_resource_provider = '苏州市公安局'
...
if __name__ == '__main__':
    ...
    apis = get_mongo_res(mongo_client, 'data_share_db', 'api_data_resource',
                          {'info_resource_id':{'$in': info_resource_ids}},
                          {'_id':1, 'api_name':1, 'info_resource_id':1})
    for item in apis:
    ...
    files = get_mongo_res(mongo_client, 'data_share_db', 'file_data_resource',
                         {'info_resource_id': {'$in': info_resource_ids}},
                         {'_id': 1, 'file_name': 1, 'info_resource_id': 1})
    for item in files:
        resource_ids.append(str(item['_id']))
        resource_id_to_info_dict[str(item['_id'])] = {
    ...
    tables = get_mongo_res(mongo_client, 'data_share_db', 'table_data_resource',
                          {'info_resource_id': {'$in': info_resource_ids}},
                          {'_id': 1, 'table_name': 1, 'info_resource_id': 1})
    for item in tables:
        resource_ids.append(str(item['_id']))
        resource_id_to_info_dict[str(item['_id'])] = {
    ...
    for apply in applies:
        ...
        info_resource_name = info_resource_id_to_name_dict.get(info_resource_id, '')

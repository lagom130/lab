...
if __name__ == '__main__':
    ...
    applies = get_mongo_res(mongo_client, 'data_share_db', 'data_resource_apply',
                            {'data_creater_dep_name':'常熟市'},
                            {'_id': 1, 'resource_id': 1, 'share_resource_id': 1, 'resource_name': 1, 'apply_type':1,'code': 1, 'status': 1,
                             'dep_name': 1, 'resource_name': 1 ,'data_creater_dep_name':1, 'apply_time': 1 ,'admin_audit_time':1, 'provider_audit_time':1,'provider_audit_time_length':1})
    ...
    row_list.append(['申请主键','resourceId','shareResourceId','申请流水号',
                     '申请方','资源名称','资源类型','提供方','申请状态','申请时间','平台管理员审核时间','提供方审核时间','提供方审核时效'])
    for apply in applies:
        ...
        latest_record_data_status = latest_record.get('data_status', '')
        if latest_record_data_status == 'TABLE|auditReject' or latest_record_data_status == 'API|auditReject' or latest_record_data_status == 'FILE|auditReject':
            status = dict_key_to_value(apply_status_dict, 'auditReject', '')
        row_list.append([
            str(apply['_id']),
            ...
            dict_key_to_value(resource_type_dict, apply.get('apply_type', ''), ''),
            apply.get('data_creater_dep_name', ''),
            status,
            timestamp_to_date_str(apply.get('apply_time', '')),
            timestamp_to_date_str(apply.get('admin_audit_time', '')),
            timestamp_to_date_str(apply.get('provider_audit_time', '')),
            apply.get('provider_audit_time_length', '')
        ])

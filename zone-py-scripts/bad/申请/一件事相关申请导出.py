...
if __name__ == '__main__':
    ...
    regex = re.compile(context)
    applies = get_mongo_res(mongo_client, 'data_share_db', 'data_resource_apply',
                            {'$or':[{'origin_incident': regex},{'use_range': regex},{'resource_name': regex}]},
    ...
    share_resource_ids = set()
    for apply in applies:
        share_resource_ids.add(apply['share_resource_id'])
    resources = get_mysql_res(
        "select id, info_resource_id, info_resource_name from data_share_db.share_data_resource where id in (%s)" % get_arr_str(share_resource_ids))
    res_dict = {}
    info_resource_ids = set()
    for res in resources:
        res_dict[res['id']] = res
        info_resource_ids.add(res['info_resource_id'])
    info_resources = get_mysql_res("select id, info_resource_name,info_resource_provider from cmp_catalog.info_resource_platform where id in (%s)" % get_arr_str(info_resource_ids))
    info_res_dict = dict()
    for info_res in info_resources:
        info_res_dict[info_res['id']] = info_res
    row_list = []
    row_list.append(['申请主键','resourceId','shareResourceId','目录主键','所属目录','提供方','资源名称','资源类型','申请流水号','申请状态','申请部门','业务系统名称','业务系统IP','系统部署地址','申请事由','申请使用范围','申请时间'])
    for apply in applies:

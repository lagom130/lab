...
def get_province_info_resources_to_info_resource_global_info(info_resource_global_dict):
    sql = "select id, dept_info_resource_code, base_info_resource_code, theme_info_resource_code, info_resource_life_cycle, report_status, info_resource_name, org_name, org_code,info_resource_name,source_departments_text from cmp_catalog.info_resource_platform where regioncode = '%s' order by update_date desc"
...
def get_info_resource_sheet_row_data_list(info_resource_global_dict):
            ...
            for entity_index in range(row_length):
                ...
                sheet_data_row_list.append([
                    ...
                    entity_to_cell_convert_by_dict(entity_index, info_res_life_cycle_dict, item,
                                                   ...
                                                   ''),
                    # 省级上报 report_status
                    entity_to_cell_convert_by_dict(entity_index, bool_str_dict, item, 'report_status'),
                    # 部门名称 org_name
                    entity_to_cell(entity_index, item, 'org_name'),
                    # 统一社会信用代码 org_code
                    entity_to_cell(entity_index, item, 'org_code'),
                    ...
                    # 共享类型 share_type
                    entity_to_cell_convert_by_dict(entity_index, item_share_type_dict, item, 'share_type', None),
                    # 共享方式 share_mode
...
def get_file_resource_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict, report_status_dict):
    ...
    for item in file_resources:
        ...
        report_status = dict_key_to_value(report_status_dict, report_status_dict.get(str(item.get('_id')), ''))
        ...
        for entity_index in range(row_length):
            ...
            sheet_data_row_list.append([
                ...
                # 提供方统一社会信用代码
                entity_to_cell(entity_index, info_res, 'org_code'),
                # 提供方部门名称
                entity_to_cell(entity_index, info_res, 'org_name'),
                # 文件资源名称
...
def get_api_resource_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict, report_status_dict):
    api_resources = get_mongo_res(mongo_client, 'data_share_db', 'api_data_resource',
                                  ...
                                  {'_id': 1, 'info_resource_id': 1, 'info_resource_name': 1, 'status': 1, 'api_name': 1,
                                   ...
                                   "use_range": 1, "inner_audit": 1, "other_files": 1})
    sheet_data_row_list = []
    for item in api_resources:
        ...
        attachments = ",".join([attachment.get('file_name', '') for attachment in item.get('other_files', [])])
        sheet_data_row_list.append([
            # 资源目录代码
            ...
            report_status,
            # 提供方统一社会信用代码
            info_res.get('org_code', None),
            # 提供方部门名称
            info_res.get('org_name', None),
            # 接口名称
            item.get('api_name', ''),
            # 接口地址
            item.get('api_url', ''),
            # 匹配模式
            item.get('url_map_mode', ''),
            # 接口描述
            item.get('api_desc', ''),
            # 请求方式
            item.get('api_method', ''),
...
def get_table_resource_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict, report_status_dict):
    ...
    for item in table_resources:
        ...
        for entity_index in range(row_length):
            ...
            sheet_data_row_list.append([
                ...
                # 字段名称
                info_item.get('item_name', ''),
                entity_to_cell(entity_index, info_item, 'item_name'),
                # 字段描述
                info_item.get('item_desc', ''),
                # 字段长度
                info_item.get('item_length', ''),
...
if __name__ == '__main__':
    ...
    wb = Workbook()
    # sheet1 信息资源目录表
    print('export info resource to xlsx started at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    info_res_sheet_title = ['资源目录代码', '状态', '省级上报', '部门名称', '统一社会信用代码', '信息资源名称',
                            ...
                            '不开放依据', '详细说明', '目录分类（按行业）', '目录分类（按主题）', '所属领域']
    sheet_data_row_list = get_info_resource_sheet_row_data_list(info_resource_global_dict)
    write_data_to_sheet(wb.active, '信息资源目录表', info_res_sheet_title, sheet_data_row_list)
    # 补上省级目录信息（给后面用）
    get_province_info_resources_to_info_resource_global_info(info_resource_global_dict)
    print('export info resource to xlsx ended at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    # sheet2 文件资源表
    print('export file resource to xlsx started at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    sheet_data_row_list = get_file_resource_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict,
                                                                data_res_report_dict)
    write_data_to_sheet(wb.create_sheet(), '文件资源表', file_res_sheet_title, sheet_data_row_list)
    # 补上省级资源信息（给后面用）
    get_province_file_resources_to_data_resource_global_info(data_resource_global_dict)
    print('export file resource to xlsx ended at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    # # sheet3 接口资源表
    print('export api resource to xlsx started at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    sheet_data_row_list = get_api_resource_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict,
                                                               data_res_report_dict)
    write_data_to_sheet(wb.create_sheet(), '接口资源表', api_res_sheet_title, sheet_data_row_list)
    # 补上省级资源信息（给后面用）
    get_province_api_resources_to_data_resource_global_info(data_resource_global_dict)
    print('export api resource to xlsx ended at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    # # sheet4 库表资源表
    print('export table resource to xlsx started at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    sheet_data_row_list = get_table_resource_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict,
                                                                 data_res_report_dict)
    write_data_to_sheet(wb.create_sheet(), '库表资源表', table_res_sheet_title, sheet_data_row_list)
    # 补上省级资源信息（给后面用）
    get_province_table_resources_to_data_resource_global_info(data_resource_global_dict)
    print('export table resource to xlsx ended at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    # sheet5 资源申请手里及授权时长表
    print('export apply to xlsx started at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    apply_sheet_title = [
        "申请流水号", "资源名称", "资源类型", "资源目录代码", "资源目录名称", "提供方统一社会信用代码",
        "提供方部门名称", "申请方统一社会信用代码", "申请方部门名称", "发起时间", "状态", "平台管理员受理审批完成时间",
        "受理审批时长", "提供部门授权审批完成时间", "授权审批时长"
    ]

    apply_sheet_data_row_list = get_apply_sheet_row_data_list(info_resource_global_dict, data_resource_global_dict)
    write_data_to_sheet(wb.create_sheet(), '资源申请手里及授权时长表', apply_sheet_title, apply_sheet_data_row_list)
    print('export apply to xlsx ended at:' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    # 将工作簿保存到 xlsx 文件
    wb.save(filename)

    ### 发送邮件
    send_mail('smtp.163.com', 25, 'rikurobot@163.com', 'BDDHTVARWQRQFPFN', receivers,
              Header(

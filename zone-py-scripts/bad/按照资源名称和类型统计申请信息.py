...
api_names = [
    ...
    "苏州人口库-身份验证",
    "苏州人口库-户籍人口信息查询",
    "苏州人口库-公安内部政务网按街路巷查询地址",
    "苏州人口库-公安内部政务网地址查询",
    "苏州人口库-公安内部政务网按派出所查询地址",
    "苏州人口库_查询同户人员",
    "苏州人口库-人口基础信息查询",
    "苏州人口库-苏州标准地址查询",
    "苏州人口库-人口户籍信息校验查询",
...
if __name__ == '__main__':
    ...
    # info_resources = []
    # for code in codes:
    #     info_resources.extend(get_mysql_res("select id, info_resource_name from cmp_catalog.info_resource_platform where info_resource_provider='"+info_resource_provider+"' and "+code_field_name+" like '"+code+"%'"))
    # info_resource_ids = []
    # info_resource_id_to_name_dict = {}
    # for item in info_resources:
    #     info_resource_ids.append(item['id'])
    #     info_resource_id_to_name_dict[item['id'] ] = item['info_resource_name']


    resource_ids = []
    resource_id_to_info_dict = {}

    if len(api_names)>0:
        apis = get_mongo_res(mongo_client, 'data_share_db', 'api_data_resource',
                             {'api_name': {'$in': api_names}},
                             {'_id': 1, 'api_name': 1, 'info_resource_id': 1, 'info_resource_name': 1})
        for item in apis:
            resource_ids.append(str(item['_id']))
            resource_id_to_info_dict[str(item['_id'])] = {
                'resource_name': item.get('api_name', None),
                'resource_type': '接口',
                'info_resource_id': item.get('info_resource_id', ''),
                'info_resource_name': item.get('info_resource_name', ''),
            }
    if len(file_names) > 0:
        files = get_mongo_res(mongo_client, 'data_share_db', 'file_data_resource',
                            {'file_name': {'$in': file_names}},
                            {'_id': 1, 'file_name': 1, 'info_resource_id': 1, 'info_resource_name': 1})
        for item in files:
            resource_ids.append(str(item['_id']))
            resource_id_to_info_dict[str(item['_id'])] = {
                'resource_name': item.get('file_name', None),
                'resource_type': '文件',
                'info_resource_name': item.get('info_resource_name', ''),
            }
    if len(table_names) > 0:
        tables = get_mongo_res(mongo_client, 'data_share_db', 'table_data_resource',
                            ...
                            {'_id': 1, 'table_name': 1, 'info_resource_id': 1, 'info_resource_name': 1})
        for item in tables:
            resource_ids.append(str(item['_id']))
            resource_id_to_info_dict[str(item['_id'])] = {
                'resource_name': item.get('table_name', None),
                'resource_type': '库表',
                'info_resource_id': item.get('info_resource_id', ''),
                'info_resource_name': item.get('info_resource_name', ''),
            }
    applies = get_mongo_res(mongo_client, 'data_share_db', 'data_resource_apply',
                  ...
                  {'_id': 1, 'resource_id':1, 'resource_name': 1, 'code': 1, 'status':1,
                   'dep_name': 1, 'business_sys_info': 1, 'origin_incident': 1,'apply_time':1})
    row_list = []
    row_list.append(['申请主键','资源主键','目录主键','目录名称','资源名称','资源类型','申请流水号','申请状态','申请部门','业务系统名称','业务系统IP','系统部署地址','申请事由','申请时间'])
    for apply in applies:
        resource_id = apply.get('resource_id','')
        resource = resource_id_to_info_dict.get(resource_id, {})
        info_resource_id = resource.get('info_resource_id', '')
        info_resource_name = resource.get('info_resource_name', '')
        ...
        row_list.append([
            str(apply['_id']),
            str(resource_id),
            str(info_resource_id),
            info_resource_name,
            resource.get('resource_name', apply.get('resource_name', '')),

...
def copy_old_dir(old_path, new_path):
    file_list = os.listdir(old_path)
    for item in file_list:
        old_item_path = old_path + '\\' + item
        new_item_path = new_path + '\\' + item
        if os.path.exists(old_item_path):
            if os.path.isdir(old_item_path):
                ...
                copy_old_dir(old_item_path, new_item_path)
            else:
                if not os.path.exists(new_path):
                    copy_file(old_item_path, new_item_path)
...
def clone(config):
    ...
    for target_project in target_projects:
        ...
        old_target_project_root_path = None
        print('****[project copy]-[start]: ' + original_project_root_path + ' -> ' + target_project_root_path)
        # 1. 复制项目
        if os.path.exists(target_project_root_path):
            old_target_project_root_path = target_project_root_path + "-clone-bak"
            os.rename(target_project_root_path, old_target_project_root_path)

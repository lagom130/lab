...
def is_diff(open_resource, info_resource):
    ...
    for open_info_item in open_info_items:
        ...
        info_resource_info_item = next((info_item for info_item in info_resource_info_items if
                                        re.sub(r'\\.', '', info_item.get('infoItemName', '')) ==  re.sub(r'\\.', '', info_item_name)),None)
...
if __name__ == '__main__':
    ...
    t =data_dict.get('30701505833010250000000000/000008')
    resource_code = '30701505003030011000000005/000023'
    open_resource = data_dict.get(resource_code)
    info_resources_by_name = [info_resource for info_resource in info_resources if
                              info_resource.get('info_resource_name') == open_resource['resource_name']]
    info_resource = next((info_resource for info_resource in info_resources_by_name if
                          info_resource.get('dept_info_resource_code') == resource_code or info_resource.get(
                              'base_info_resource_code') == resource_code or info_resource.get(
                              'theme_info_resource_code') == resource_code), None)
    if info_resource is None:
        if len(info_resources_by_name) == 1:
            info_resource = info_resources_by_name[0]
            remarks.append([open_resource['open_id'], open_resource['resource_status'], resource_code,
                            open_resource['resource_name'], '编码不正确', info_resource['id']])
        if len(info_resources_by_name) > 1:
            remarks.append([open_resource['open_id'], open_resource['resource_status'], resource_code,
                            open_resource['resource_name'], '编码不正确,同名对应多个目录',
                            str([info_resource['id'] for info_resource in info_resources_by_name])])
        if len(info_resources_by_name) == 0:
            ...
            if len(info_resources_by_code) == 0:
                ...
                not_found_open_resources.append(
                    [open_resource['open_id'], open_resource['resource_status'], open_resource['resource_name'],
                     str(open_resource)])
            if len(info_resources_by_code) > 1:
                remarks.append([open_resource['open_id'], open_resource['resource_status'], resource_code,
                                open_resource['resource_name'], '名称不正确,编码对应多个目录',
                                str([info_resource['id'] for info_resource in info_resources_by_code])])
    if info_resource is not None:
        ...
        if flag:
            if info_resource is None:
                not_found_open_resource_ids.append(open_resource['open_id'])
                not_found_open_resources.append(
                    [open_resource['open_id'], open_resource['resource_status'], str(open_resource)])
            else:
    ...
    ws2.append(['开放平台ID', '开放平台状态', '共享平台目录ID', '原因', '开放平台开放类型', '共享平台生命周期',
                '共享平台是否开放', '共享平台开放类型', '开放平台信息项', '共享平台信息项'])
    # 将数据写入工作表

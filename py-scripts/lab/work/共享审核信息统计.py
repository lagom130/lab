...
info_resource_sql = "select id, info_resource_name, info_resource_provider,create_date,info_resource_life_cycle, is_open, regioncode, info_item_desc_detail from cmp_catalog.info_resource_platform WHERE regioncode IN (%s)"

# 共享资源mysql查询, 第一个占位符为region_id 第二个占位符为过滤部门列表
data_share_sql = "SELECT m.id as id, m.resource_id as resource_id, m.resource_type as resource_type, m.resource_name as resource_name, m.info_resource_name as info_resource_name, m.status as status, m.creater_dep_name as creater_dep_name, m.created_time as created_time, m.region_id as region_id FROM `data_share_db`.`share_data_resource` m WHERE m.region_id IN (%s) AND creater_dep_id NOT IN (%s)"
...
def get_region_ids(codes):
    region_catalogs = mongo_client["cmp_catalog"]["map_catalog"].find(
        {'mapCatalogCode': {'$in': codes}, 'mapCatalogType': 'region'})
    region_ids = []
...
def get_cmp_log():
    ...
    logs_dict = {}
    operation_records = mongo_client["cmp_catalog"]["infoResOPLogEntity"].find({})
...
def get_operation_records():
    ...
    logs_dict = {}
    operation_records = mongo_client["data_share_db"]["operation_record"].find({})
...
if __name__ == '__main__':
    ...
    write_csv_to_zip(
        get_info_resource_audit_list(info_resource_sql % get_arr_str(quxian_region_map_catalog_codes), cmp_logs_dict,
                                     info_resource_resource_ids_dict),
        '目录发布审核(区县).csv', zip_file)
    ...
    filter_dep_str = get_arr_str(filter_dep)
    # 统计 共享资源发布审核(市级)
    print('analysis share data resource (suzhoushi) started')
    region_codes_str = get_arr_str(get_region_ids(shiji_region_map_catalog_codes))
    write_csv_to_zip(get_data_share_audit_list(data_share_sql % (region_codes_str, filter_dep_str), ds_logs_dict,
                                               data_resource_dict), '共享资源发布审核(市级).csv', zip_file)
    ...
    region_codes_str = get_arr_str(get_region_ids(quxian_region_map_catalog_codes))
    write_csv_to_zip(get_data_share_audit_list(data_share_sql % (region_codes_str, filter_dep_str), ds_logs_dict,
                                               data_resource_dict), '共享资源发布审核(区县).csv', zip_file)
    ...
    write_csv_to_zip(get_apply_audit_list(ds_logs_dict, data_resource_dict), '共享资源申请审核(所有状态的).csv', zip_file)
